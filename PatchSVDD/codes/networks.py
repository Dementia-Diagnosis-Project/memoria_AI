...
class Encoder(nn.Module):
    def __init__(self, K, D=64, bias=True):
        ...
        # MRI 사진은 흑백 사진이므로 3채널을 1채널로 변경
        # Conv2d의 0이 padding 값을 의미
        self.conv1 = nn.Conv2d(1, 64, 5, 32, 5, bias=bias)
    ...
    def fpath_from_name(name):
        return os.path.join(DATASET_PATH, f'ckpts/{name}/encoder_nohier.pt')

# 64*64 패치를 32*32 패치 4개로 분할한 다음 enc.enc로 임데빙
...
class EncoderDeep(nn.Module):
    def __init__(self, K, D=64, bias=True):
        ...
        # MRI 사진은 흑백 사진이므로 3채널을 1채널로 변경
        self.conv1 = nn.Conv2d(1, 32, 3, 2, 0, bias=bias)
    ...
    @staticmethod
    def fpath_from_name(name):
        return os.path.join(DATASET_PATH, f'ckpts/{name}/encdeep.pt')
...
class EncoderHier(nn.Module):
    ...
    def forward(self, x):
        # 64*64 패치를 32*32 패치 4개로 분할한 다음 concat 한 다음 representation 생성
        h = forward_hier(x, self.enc, K=self.K)

        #conv1 + Leaky_ReLU + conv2 + tanh --> f_big
        h = self.conv1(h)
        h = F.leaky_relu(h, 0.1)
    ...
    @staticmethod
    def fpath_from_name(name):
        return os.path.join(DATASET_PATH, f'ckpts/{name}/enchier.pt')
...
class PositionClassifier(nn.Module):
    ...
    def fpath_from_name(self, name):
        return os.path.join(DATASET_PATH, f'ckpts/{name}/position_classifier_K{self.K}.pt')

    @staticmethod
    # c : classifier, enc : 인코더
    ...
    # 기준 패치 representation과 랜덤 패치의 representation
    # 이 두 rerpesentation을 입력해서 랜덤 패치의 위치를 출력하는 거로 이해하자
